        ...
        With xlWs
            ...
            For i = 0 To recCount
                ...
                If rs_EXCEL.Tables("RESULT").Rows(i).Item(sod_shpstr_enu) <> rs_EXCEL.Tables("RESULT").Rows(i).Item(sod_shpend_enu) Then
                    .Cells(intGroup + DtlRow + i, indexCol + 11) = Format(CDate(rs_EXCEL.Tables("RESULT").Rows(i).Item(sod_shpstr_enu)), "MM/dd/yyyy") & " - " & Format(CDate(rs_EXCEL.Tables("RESULT").Rows(i).Item(sod_shpend_enu)), "MM/dd/yyyy")   'Ship Start Date
                Else
                    .Cells(intGroup + DtlRow + i, indexCol + 11) = "'" & CStr(Format(CDate(rs_EXCEL.Tables("RESULT").Rows(i).Item(sod_shpstr_enu)), "MM/dd/yyyy"))
                End If
                ...
                '''
                '.Cells(intGroup + DtlRow + i, indexCol + 12) = rs_EXCEL.Tables("RESULT").Rows(i).Item(sod_candat_enu).ToString
                '                .Range(.Cells(intGroup + DtlRow + i, indexCol + 12)).Formula = "@"
                '   .Range(.Cells(intGroup + DtlRow + i, indexCol + 12)).Formula = "=text(now(),""mmm dd yyyy"")"

                '   .Range(.Cells(intGroup + DtlRow + i, indexCol + 12)).Formula = "@"
                '.Cells(intGroup + DtlRow + i, indexCol + 12) = IIf(Trim(rs_EXCEL.Tables("RESULT").Rows(i).Item(sod_candat_enu).ToString) = "", "", CStr(CDate(rs_EXCEL.Tables("RESULT").Rows(i).Item(sod_candat_enu).ToString)))

                If Trim(rs_EXCEL.Tables("RESULT").Rows(i).Item(sod_candat_enu).ToString) = "" Then
                    .Cells(intGroup + DtlRow + i, indexCol + 12) = ""
                Else
                    .Cells(intGroup + DtlRow + i, indexCol + 12) = CStr(CDate(rs_EXCEL.Tables("RESULT").Rows(i).Item(sod_candat_enu).ToString))
                End If

                '                .Cells(intGroup + DtlRow + i, indexCol + 12) = Format(rs_EXCEL.Tables("RESULT").Rows(i).Item(sod_candat_enu).ToString, "yyyy/mm/dd")

                '.Cells(intGroup + DtlRow + i, indexCol + 12).

                '.Cells(intGroup + DtlRow + i, indexCol + 12) = Format(rs_EXCEL.Tables("RESULT").Rows(i).Item(sod_candat_enu), "MM/dd/yyyy")
            ...
            '20140818
            .Range(.Cells(DtlRow, indexCol + 12), .Cells(intGroup + DtlRow + recCount + 1, indexCol + 12)).NumberFormatLocal = "mm/dd/yyyy" 'Unit Price
