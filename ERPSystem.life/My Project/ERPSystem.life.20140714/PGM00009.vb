...
SCIssdatTO & "','" & _
        ...
        'If rdoIn.Checked = True Then
        '    Dim cate As String
        '    Dim OrdPlusStkQty As Integer

        '    cate = rs_PKIMBAIF.Tables("RESULT").Rows(0).Item("pib_cate")
        '    OrdPlusStkQty = stkqty + ordqty

        '    Dim dr() As DataRow
        '    dr = rs_syswasge.Tables("RESULT").Select("pwa_code = '" & cate & "' and pwa_qtyfrm <= " & OrdPlusStkQty & " and pwa_qtyto >= " & OrdPlusStkQty)

        '    If dr.Length <> 0 Then
        '        If dr(0)("pwa_um") = "%" Then

        '            txtPkgWastPer.Text = Fix(dr(0).Item("pwa_wasage"))
        '            txtWasQty.Text = Math.Round(OrdPlusStkQty * dr(0).Item("pwa_wasage") / 100)
        ...
        '    unitprice = txtUnitPrc.Text


        '    txtTtlOrdQty.Text = OrdPlusStkQty + wasqty

        '    Dim ttlordqty As Integer = txtTtlOrdQty.Text

        '    txtTtlAmt.Text = unitprice * ttlordqty
        ...
        '    Dim wasqty As Integer
        '    Dim tmp_stkqty As Integer

        '    tmp_stkqty = txtStkQty.Text * -1
        '    wasqty = txtWasQty.Text
        '    ordqty = txtOrdQty.Text

        '    If wasqty + ordqty < tmp_stkqty Then
        ...
        '    txtTtlOrdQty.Text = OrdPlusStkQty + wasqty + stkqty
        '    Dim ttlordqty As Integer = txtTtlOrdQty.Text
        ...
        '    gspStr = "sp_select_SCORDDTL_PKG09 '" & cboCoCde.Text & "','" & txtScFrm.Text & "','" & txtScTo.Text & "'"
        '    rtnLong = execute_SQLStatement(gspStr, rs_TOSCDETAIL, rtnStr)
        '    If rtnLong <> RC_SUCCESS Then
        '        Cursor = Cursors.Default
        '        MsgBox("Error on loading txtToNo_KeyPress sp_select_TOORDDTL_PKG02 :" & rtnStr)
        ...
        '    txtRemain.Text = txtOrdQty.Text


        'Else

        '    Me.Cursor = Cursors.Default
        '    MsgBox("SC or TO not found.")
                ...
                For i As Integer = 0 To rs_TOSCDETAIL_tmp_sorttable.Rows.Count - 1
                    ...
                    If rs_TOSCDETAIL_tmp_sorttable.Rows(i).Item("Gen") = "Y" And rs_TOSCDETAIL_tmp_sorttable.Rows(i).Item("ordqty") <> 0 Then
                        ...
                        If FirstTime = True Then
                            ...
                            Dim pod_cate As String = Replace(rs_PKIMBAIF.Tables("RESULT").Rows(0).Item("pib_cate"), "''", "'")
                            ...
                            Dim pod_EInchL As Decimal = Replace(rs_PKIMBAIF.Tables("RESULT").Rows(0).Item("pib_EInchL"), "''", "'") '
                            Dim pod_EInchW As Decimal = Replace(rs_PKIMBAIF.Tables("RESULT").Rows(0).Item("pib_EInchW"), "''", "'")
                            Dim pod_EInchH As Decimal = Replace(rs_PKIMBAIF.Tables("RESULT").Rows(0).Item("pib_EInchH"), "''", "'")
                            ...
                            Dim pod_matral As String = Replace(rs_PKIMBAIF.Tables("RESULT").Rows(0).Item("pib_matral"), "''", "'")
                            Dim pod_tiknes As String = Replace(rs_PKIMBAIF.Tables("RESULT").Rows(0).Item("pib_tiknes"), "''", "'")
                            Dim pod_prtmtd As String = Replace(rs_PKIMBAIF.Tables("RESULT").Rows(0).Item("pib_prtmtd"), "''", "'")
                            Dim pod_clrfot As String = Replace(rs_PKIMBAIF.Tables("RESULT").Rows(0).Item("pib_clrfot"), "''", "'")
                            Dim pod_clrbck As String = Replace(rs_PKIMBAIF.Tables("RESULT").Rows(0).Item("pib_clrbck"), "''", "'")
                            ...
                            Dim pod_addres As String = dr_vendor(0).Item("vci_address").ToString
                           ...
                           pod_Reqseq & ",'" & pod_Conmak & "'," & pod_bonqty & ",'" & gsUsrID & "'"
                        ...
                        Dim reqdtl_addres As String = dr_vendor(0).Item("vci_address").ToString
                        ...
                        Dim reqdtl_wasper As Decimal = 0
                        Dim reqdtl_wasqty As Integer = 0
                        Dim reqdtl_ttlordqty As Integer = rs_TOSCDETAIL_tmp_sorttable.Rows(i).Item("ordqty")
                        Dim reqdtl_untprc As Decimal = round(Val(txtUnitPrc.Text), 5)
                        Dim reqdtl_ttlamtqty As Decimal = round(rs_TOSCDETAIL_tmp_sorttable.Rows(i).Item("ordqty") * round(Val(txtUnitPrc.Text), 5), 2)
                        ...
                        gspStr = "sp_insert_PKREQDTL_09 '" & reqdtl_cocde & "','" & NewPKGREQHDR & "'," & 0 & ",'" & reqdtl_itemno & "','" & reqdtl_assitm & "','" & reqdtl_tmpitmno & "','" & _
                                                   ...
                                                   reqdtl_ScToNo & "'," & reqdtl_ScToSeq & ",'" & gsUsrID & "'"
