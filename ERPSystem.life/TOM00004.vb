    ...
    Private Sub mmdFindClick()
        ...
        If txtToNo.Text <> "" Then
            ...
            If rtnLong <> RC_SUCCESS Then
                MsgBox("Error on loading TOM00004 mmdFind_Click sp_select_TOITMDTL: " & rtnStr)
            ...
            Me.Cursor = Windows.Forms.Cursors.Default
            If rtnLong <> RC_SUCCESS Then
                MsgBox("Error on loading TOM00004 mmdFind_Click sp_select_TOITMSUM: " & rtnStr)
            Else
                If rs_TOITMSUM.Tables("RESULT").Rows.Count > 0 Then
                    ...
                    Dim dv As DataView = rs_TOITMSUM.Tables("RESULT").DefaultView
                    If Not dv.Count = 0 Then
                        dv.Sort = "tis_upddat desc"
                        Dim drv As DataRowView = dv(0)
                        Me.StatusBar.Items("lblRight").Text = Format(drv.Item("tis_credat"), "MM/dd/yyyy") & " " & Format(drv.Item("tis_upddat"), "MM/dd/yyyy") & " " & drv.Item("tis_updusr")
        ...
        If rdbitmno.Checked = True Then
            ...
            gspStr = "sp_select_TOITMDTL '" & gsCompany & "','" & Replace(UCase(cboCus1No.Text), "'", "''") & "','" & Replace(UCase(cboCus2No.Text), "'", "''") & "','" & Replace(UCase(txtItmNo.Text), "'", "''") & "'"
            'Replace(UCase(txtCusnam.Text), "'", "''")
            Me.Cursor = Windows.Forms.Cursors.WaitCursor
            rtnLong = execute_SQLStatement(gspStr, rs_TOITMDTL, rtnStr)
        ...
        Else
            ...
            If rtnLong <> RC_SUCCESS Then
                MsgBox("Error on loading TOM00004 mmdFind_Click sp_select_TOITMDTL_VN: " & rtnStr)
                Me.Cursor = Windows.Forms.Cursors.Default
                Exit Sub
            Else
                If rs_TOITMDTL.Tables("RESULT").Rows.Count = 0 Then
            ...
            Me.Cursor = Windows.Forms.Cursors.Default
            If rtnLong <> RC_SUCCESS Then
                MsgBox("Error on loading TOM00004 mmdFind_Click sp_select_TOITMSUM_VN: " & rtnStr)
            Else
                If rs_TOITMSUM.Tables("RESULT").Rows.Count > 0 Then
                    Call Display()
                    Call setStatus("Updating")
...
result:
    ...
    Private Sub Display_Sum()
        ...
        With grdSum
            ...
            .Columns(1).Width = 75
            .Columns(1).HeaderCell.Value = "Pri Cust No."
            .Columns(2).Width = 75
            .Columns(2).HeaderCell.Value = "Sec Cust No."
            .Columns(3).Width = 70
            ...
            .Columns(7).Width = 110
            .Columns(7).HeaderCell.Value = "Temp Item No."
            .Columns(8).Width = 70
            .Columns(8).HeaderCell.Value = "Vendor No."
            .Columns(9).Width = 110
            .Columns(9).HeaderCell.Value = "Vendor Item No."
            .Columns(10).Width = 70
            ...
            .Columns(13).Width = 70
            .Columns(13).HeaderCell.Value = "O/S Qty"
    ...
    Private Sub Display_Dtl()
        ...
        With grdDtl
            ...
            .Columns(12).HeaderCell.Value = "SO Qty"
            .Columns(13).Width = 90
            .Columns(13).HeaderCell.Value = "TO No"
            .Columns(14).Width = 70
            .Columns(14).HeaderCell.Value = "TO Seq"
            .Columns(15).Width = 70
            .Columns(15).HeaderCell.Value = "Verson No."
            .Columns(16).Width = 90
            .Columns(16).HeaderCell.Value = "Quotation No."
            .Columns(17).Width = 70
            .Columns(17).HeaderCell.Value = "QU Seq"
            .Columns(18).Visible = False
    ...
    Private Sub fillCus2No(ByVal prmcus As String)
        ...
        '    Dim drCUBASINF_S() As DataRow = rcboCus1Nos_CUBASINF_S.Tables("RESULT").Select("cbi_cus1no = '" + Split(prmcus, " - ")(0) + "'")
        '    For i As Integer = 0 To drCUBASINF_S.Length - 1
        '        cboCus2No.Items.Add(Trim(drCUBASINF_S(i).Item("cbi_cus2no").ToString) + " - " + Trim(drCUBASINF_S(i).Item("cbi_cussna").ToString))
        '    Next
        'Else
    ...
    End Sub
    Public Sub callByTOM01(ByVal TONo As String)

        txtToNo.Text = TONo

        'Hints: In .net, Shown event is called after Load event
        AddHandler Me.Shown, AddressOf callBTOM01AfterLoading
        Me.ShowDialog()
    End Sub

    Private Sub callBTOM01AfterLoading()
        mmdFindClick()
        mmdClear.Enabled = False
        RemoveHandler Me.Shown, AddressOf callBTOM01AfterLoading
    End Sub
End Class
