Public Class SYS00004
    ...
    Private Sub setDataRowAttr()
        ...
        If Not rs_rights.Tables("RESULT") Is Nothing Then
            ...
            If Not rs_rights.Tables("RESULT").DefaultView.Count = 0 Then
                rs_rights.Tables("RESULT").DefaultView.Sort = "yur_upddat desc"
                Dim drv As DataRowView = rs_rights.Tables("RESULT").DefaultView(0)
                Me.StatusBar.Items("lblRight").Text = Format(drv.Item("yur_credat"), "MM/dd/yyyy") & " " & Format(drv.Item("yur_upddat"), "MM/dd/yyyy") & " " & drv.Item("yur_updusr")

                rs_rights.Tables("RESULT").DefaultView.Sort = Nothing
            Else
        ...
        If Not e.RowIndex = -1 Then
            ...
            If e.ColumnIndex = 0 Then
                ...
                If Not row.Cells("yur_lvl").Value.ToString = "" Then
                    If chkSuper.Checked() = False Then
    ...
    Private Sub DataGrid_CellValidating(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellValidatingEventArgs) Handles DataGrid.CellValidating
        ...
        If row.Cells(e.ColumnIndex).IsInEditMode Then
            ...
            If e.ColumnIndex = 4 Then
                If (row.Cells("yur_para").Value.ToString <> "") Then
    ...
    Private Sub mmdInsRow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mmdInsRow.Click
        ...
        For Each drr As DataGridViewRow In DataGrid.Rows
            If IsDBNull(drr.Cells(4).Value) Then
                ...
                createComboBoxCell(DataGrid.CurrentCell)
                createComboBoxCell(drr.Cells(5))
                DataGrid.BeginEdit(True)
    ...
    Private Sub cmdApplyAllusrgrp_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdApplyAllusrgrp.Click
        Dim dt_rights, dt_super As DataTable
        Dim dr, dr_rights(), dr_super(), selected_dr_rights(), selected_dr_super() As DataRow
        Dim list_super, list_rights As New ArrayList

        Dim selected_module_long As String = CboModule.SelectedItem
        Dim selected_module As String = Split(selected_module_long, " - ")(0).Trim
        Dim selected_chksuper As Boolean = chkSuper.Checked
        ...
        selected_dr_rights = dt_rights.Select("yur_doctyp = '" & selected_module & "'")
        selected_dr_super = dt_super.Select("yur_doctyp = '" & selected_module & "'")



        'Create list to store super user
        For Each tmp_row As DataRow In dt_super.Rows
            list_super.Add(tmp_row.Item("yur_doctyp").ToString)
        Next


        For Each current_obj As Object In CboModule.Items
            Dim current_module_long As String = current_obj.ToString
            Dim current_module As String = Split(current_module_long, " - ")(0).Trim

            If current_module_long = selected_module_long Then
                'Do nothing
                Continue For
            Else
                If selected_chksuper Then
                    dr_super = dt_super.Select("yur_doctyp = '" & current_module & "' and yur_status = 'Y'")
                    If list_super.Contains(current_module) Then
                        If dr_super.Length > 0 Then
                            dr_super(0).Item("yur_status") = ""
                        End If
                    Else
                        dr = dt_super.NewRow
                        dr.Item("yur_status") = ""
                        dr.Item("yur_cogrp") = DataGridUsr.CurrentRow.Cells("yco_cogrp").Value.ToString.Trim
                        dr.Item("yur_usrid") = DataGridUsr.CurrentRow.Cells("yup_usrid").Value.ToString.Trim
                        dr.Item("yur_doctyp") = current_module
                        dr.Item("yur_lvl") = 0
                        dr.Item("yur_para") = "S"
                        dr.Item("yur_desc") = "Super User"
                        dt_super.Rows.Add(dr)
                    ...
                    dr_rights = dt_rights.Select("yur_doctyp = '" & current_module & "'")
                    If dr_rights.Length > 0 Then
                        For i = 0 To dr_rights.Length - 1
                            dr_rights(i).Item("yur_status") = "Y"
                        Next
                    End If
                Else    'selected_chksuper = false Start from here
                    If selected_dr_rights.Length > 0 Then
                        dr_super = dt_super.Select("yur_doctyp = '" & current_module & "'")
                        If dr_super.Length > 0 Then
                            dr_super(0).Item("yur_status") = "Y"
                        End If

                        dr_rights = dt_rights.Select("yur_doctyp = '" & current_module & "'")

                        If dr_rights.Length = 0 Then
                            For Each tmp_row As DataRow In selected_dr_rights
                                dr = dt_rights.NewRow
                                dr.Item("yur_status") = tmp_row.Item("yur_status").ToString.Trim
                                dr.Item("yur_cogrp") = DataGridUsr.CurrentRow.Cells("yco_cogrp").Value.ToString.Trim
                                dr.Item("yur_usrid") = DataGridUsr.CurrentRow.Cells("yup_usrid").Value.ToString.Trim
                                dr.Item("yur_doctyp") = current_module
                                dr.Item("yur_lvl") = tmp_row.Item("yur_lvl").ToString.Trim
                                dr.Item("yur_para") = tmp_row.Item("yur_para").ToString.Trim
                                dr.Item("yur_desc") = tmp_row.Item("yur_desc").ToString.Trim
                                dr.Item("yul_desc") = tmp_row.Item("yul_desc").ToString.Trim
                                dt_rights.Rows.Add(dr)
                            Next
                        Else
                            For Each tmp_row As DataRow In selected_dr_rights
                                Dim flg_not_equal As Boolean = True
                                For Each checked_row As DataRow In dr_rights
                                    Dim rtnvalue As Integer = Check_row(checked_row, tmp_row)
                                    If rtnvalue = 0 Then
                                        Continue For
                                    ElseIf rtnvalue = 1 Then
                                        flg_not_equal = False
                                        Exit For
                                    ElseIf rtnvalue = 2 Then
                                        flg_not_equal = False
                                        checked_row.Item("yur_status") = tmp_row.Item("yur_status").ToString.Trim
                                        Exit For
                                    End If
                                Next

                                If flg_not_equal = True Then
                                    dr = dt_rights.NewRow
                                    dr.Item("yur_status") = tmp_row.Item("yur_status").ToString.Trim
                                    dr.Item("yur_cogrp") = DataGridUsr.CurrentRow.Cells("yco_cogrp").Value.ToString.Trim
                                    dr.Item("yur_usrid") = DataGridUsr.CurrentRow.Cells("yup_usrid").Value.ToString.Trim
    ...
    Private Function Check_row(ByRef row1 As DataRow, ByRef row2 As DataRow) As Integer
        If row1.Item("yur_lvl") = row2.Item("yur_lvl") And _
        row1.Item("yur_para") = row2.Item("yur_para") Then

            If row1.Item("yur_status") = row2.Item("yur_status") Then
                Return 1 'identical
            Else
                Return 2
                'identical but need to change delete status
            End If
        Else
            Return 0
            'not identical
    ...
    Private Sub cmdRefresh_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdRefresh.Click
        ...
        If flgMod Then
            ...
            answer = MsgBox("You have made some change(s). If you refresh, the change(s) will be lost. Do you still want to refresh?", MsgBoxStyle.YesNoCancel)
            If answer = MsgBoxResult.Yes Then
                SYS00004_Load(Nothing, Nothing)
